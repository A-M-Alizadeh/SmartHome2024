...
const Dashboard = () => {
  ...
  const editHouse = (house_id, title, address) => {
    let userData = JSON.parse(localStorage.getItem("userData"));
    fetch(`${catalogUrl}${catalogPort}/house/updatehouse?userId=${userData.user_id}&houseId=${house_id}`, {
      method: 'PUT',
      body: JSON.stringify({
        title: title,
        address: address,
    ...
    <Grid container sx={{ height: "", backgroundColor: "#353455" }}>
      ...
      <Grid container spacing={4} flex={1} height="100%">
        {houses && houses.map((item, index) => {
          return (
            <HouseItem key={index} house={item} deleteHouse={deleteHouse} editHouse={editHouse}/>
            ...
            //         <Button variant="contained" color="primary" onClick={()=>deleteHouse(item.house_id)} style={{backgroundColor: 'darkred',fontSize: 8, marginTop: 3, marginRight: 122}}>Delete</Button>
            //         <Button variant="contained" color="primary" onClick={()=>editHouse(item.house_id)} style={{backgroundColor: 'orange',fontSize: 8, marginTop: 3}}>Edit</Button>
            //       </div>
            //     </Grid>
            //   </Box>
            // </Grid>
